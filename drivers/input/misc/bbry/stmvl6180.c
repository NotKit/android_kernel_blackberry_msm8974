...
static int vl6180_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (err >= 0) {
		vl6180_config.vl6180_reset = err;
		if (0 != expose_reset(&client->dev, vl6180_config.vl6180_reset, "vl6180_reset", 1)) {
			dev_err(&client->dev, "failed to expose reset pin");
			goto fail0;
		}
	} else
		vl6180_config.vl6180_reset = -1;
	...
	err = of_property_read_u32_array(np, "interrupts", interrupts, 2);
	if (err < 0) {
		dev_err(&client->dev, "failed to get interrupt property");
		goto fail2;
	}
	vl6180_config.vl6180_int = interrupts[0];
...
fail1:
	if (vl6180_config.vl6180_reset >= 0) {
		gpio_unexport(vl6180_config.vl6180_reset);
		gpio_free(vl6180_config.vl6180_reset);
	}
...
static int vl6180_remove(struct i2c_client *client)
{
	...
	if (config) {
		...
		if (config->vl6180_reset >= 0) {
			gpio_unexport(config->vl6180_reset);
			gpio_free(config->vl6180_reset);
		}
	}
	return 0;
}
