...
/* Compatibility for fget_light/fput_light API change in 3.18 */
static inline struct file *compat_fget_light(int fd, int *fput_needed)
{
...
static inline void compat_fput_light(struct file *file, int fput_needed)
{
	if (fput_needed)
		fput(file);
}
...
static int mdss_mdp_put_img(struct mdss_mdp_img_data *data)
{
	...
	if (data->flags & MDP_MEMORY_ID_TYPE_FB) {
		...
		compat_fput_light(data->srcp_file, data->p_need);
		data->srcp_file = NULL;
		...
		struct mdss_mdp_img_data *data)
{
	...
	if (img->flags & MDP_BLIT_SRC_GEM) {
		...
		file = compat_fget_light(img->memory_id, &data->p_need);
		if (file == NULL) {
