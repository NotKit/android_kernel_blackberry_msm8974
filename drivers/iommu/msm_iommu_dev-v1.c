				...
				struct msm_iommu_ctx_drvdata *ctx_drvdata)
{
	...
	if (drvdata->sec_id != -1) {
		...
		/*
		 * Skip non-secure IRQ registration during probe.
		 * The fault handler requires the IOMMU to be attached and
		 * powered on (checks drvdata->powered_on), so registering
		 * it here is premature. Additionally, enabling the interrupt
		 * in the GIC during probe can cause an intermittent spinlock
		 * deadlock on irq_controller_lock when racing with other
		 * GIC operations on other CPUs.
		 *
		 * The non-secure fault IRQ will still be available if needed
		 * by registering it at attach time.
		 */
		irq = platform_get_irq(pdev, 0);
		if (irq > 0)
			pr_info("non-secure irq %d available for later registration\n", irq);
	}
